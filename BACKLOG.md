...
- **VERIFIED — and the answer is that no scheduled job has ever run in production.** `vercel crons ls` reports all six as **`not deployed`** ("6 local changes pending deploy"). The cause is not the plan tier: the crons block entered `vercel.json` on 2026-07-20 in `71ef1c6`, and the live production deployment is from 2026-07-19 — it predates them by a day and has not been replaced since. So for the last ~14 days there have been no reminders, no payment expiry, no session finalization, no FX refresh, no grace/dunning advancement and no outbound email from the cron path. The empty `JobRun` table is consistent with this and is not independent evidence, since the check-in code is itself undeployed. **STILL OUTSTANDING: a production deploy** (`vercel deploy --prod`), which is an operator action and was not taken from here. Plan tier remains unconfirmed — `vercel usage` returns 404 and the account is a personal `101stewie101's projects` team, both consistent with Hobby but not proof; it only becomes the binding constraint once a deploy exists, at which point `vercel crons ls` and the `JobRun` check-ins answer it empirically.

...
- **STILL OUTSTANDING:** an operator decision on how the schedules run at all — upgrade to Pro, degrade `vercel.json` to daily schedules (which makes lesson reminders useless and leaves the new after-response trigger as the real email path), or drive the endpoints from an external scheduler such as GitHub Actions, which supports 5-minute crons on the free tier and would keep every schedule intact since the routes are already `CRON_SECRET`-authenticated and self-monitoring.

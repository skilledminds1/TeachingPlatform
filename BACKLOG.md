...
### ⚪ `SEC-18` Store OAuth profile pictures ourselves instead of allowing the provider's CDN

- [x] **Effort:** S · <½ day · **Area:** csp
- **Files:** `src/server/auth/provider-avatar.ts`, `src/server/auth/session.ts`, `src/lib/security/csp.ts`, `src/lib/security/image-signature.ts`, `scripts/backfill-provider-avatars.ts`
- **What:** Google sign-in writes an avatar_url on the googleusercontent.com CDN into the Supabase auth metadata, and syncUserFromAuth stored it verbatim on User.avatarUrl, where the public tutor listing rendered it — against an img-src allowing only self, data:, blob: and Supabase. Every teacher who signed up with Google showed an empty avatar and a console violation on /find-tutor. Not exploitable; the cost is a broken public listing. Widening img-src is the wrong fix: Google spreads profile photos across lh3–lh6.googleusercontent.com and moves between them, so the entry that covers every user is the whole Google user-content CDN on every page including the dashboards, and avatar_url is user-writable metadata (supabase.auth.updateUser), so the listing would render a third-party URL of its owner's choosing to other users. Import the bytes into the avatars bucket at sign-in behind a host allowlist that doubles as the SSRF boundary, follow redirects manually so a permitted host cannot bounce the fetch to an internal address, and check the magic bytes against the declared type. Separately, stop getCurrentUser re-reading avatarUrl from provider metadata on every request: uploadTeacherAvatar writes only the User row, so a teacher who signed in with Google and then uploaded a photo had it replaced by their Google picture on the next page view.
- **Done when:** /find-tutor renders Google-sourced avatars from our own storage with no CSP violations in the console; img-src still names no third-party host; an avatar_url pointed at a private address is never fetched; a photo the user uploaded is never overwritten from provider metadata.
...
- [x] **Effort:** S · <½ day · **Area:** subscriptions
- **Outcome:** RESOLVED BY DELETION (the second branch). Previously ticked without either branch of the "done when" actually happening — the trial stayed dead code and the docs kept advertising it. Removed: `startPaidTrial`, `TRIAL_DAYS`, the `trialing` branches in `isGrowthBlocked`/`growthBlockMessage`, the lifecycle trial-end branch and its `trialEndsAt` scan predicate, the `trialsEnded` counter, the trial notice in the billing plan selector, the `trialing` arm of the admin MRR filter and status badge, and every `trialEndsAt: null` write. `src/` now contains zero references to any of it. Migration `20260802160000_retire_subscription_trial` drained legacy rows to `active` and dropped `trialing` from the `subscription_status` enum, so the type can no longer express a state the product does not have — **0 rows were affected**, confirmed by a read-only count before applying, and `prisma migrate diff` reports no drift. The `trial_ends_at` column is deliberately retained: nothing reads or writes it, but dropping a column is irreversible and belongs with a wider schema cleanup. Docs corrected in `docs/PayFast.md` (state table and diagram) and `docs/Database.md`.
...
- **PARTIAL — code complete; blocked on a production deploy.** Job check-in recording (`JobRun`, one row per job, updated in place), a registry pairing each job with its schedule and what breaks when it stops, staleness assessed from the last SUCCESS rather than the last attempt, and readiness now returning 503 when CRON_SECRET is absent in production or any job has stalled. A 401 deliberately does not count as a check-in — otherwise the monitoring would confirm the very outage it exists to catch. The email-outbox after-response trigger is now built (`src/server/notifications/outbox-trigger.ts`): `enqueueEmail` drains via Next's `after()` so transactional mail leaves on the current response instead of waiting up to a cron tick, bounded to 10 messages, silent outside a request scope, and never able to fail the request that enqueued it — with the cron kept as the backstop, since `after()` dies with its invocation and cannot deliver a retry backed off to a future `nextAttemptAt`.
- **VERIFIED — the plan is Hobby, and the project has been UNDEPLOYABLE since 2026-07-20.** `vercel deploy --prod` is rejected outright by Vercel, before the build: *"Hobby accounts are limited to daily cron jobs. This cron expression (\*/15 \* \* \* \*) would run more than once per day."* That is the definitive answer the earlier passes could not get. It also explains `vercel crons ls` reporting all six as `not deployed`: the crons block entered `vercel.json` on 2026-07-20 in `71ef1c6`, the live production deployment is from 2026-07-19, and **every deploy attempt since has been refused for this reason** — so it is not that nobody deployed, it is that nobody could. Consequences: no reminders, no payment expiry, no session finalization, no FX refresh, no grace/dunning advancement and no cron-path email for ~14 days; and every commit from P0/P1/P3/P5 is sitting undeployed. The empty `JobRun` table is consistent but is not independent evidence, since the check-in code is itself undeployed. Production was never touched by the attempt and still serves the 2026-07-19 build.
- **ALSO BLOCKING, found while pre-flighting the deploy:** production is missing 8 variables that `assertProductionEnv` (QLT-03) treats as fatal — `RESEND_API_KEY`, `CRON_SECRET`, `TOKEN_ENCRYPTION_KEY`, and all five `LEGAL_*` values still on their placeholder defaults. `next build` skips the check by design, so a deploy that got past the cron gate would build, promote, and then 500 on every request at boot. Simulated against the real production variable list and confirmed: 8 problems. These must be set before any deploy, independently of the cron question.
- **RESOLVED — schedules moved to GitHub Actions** (`c9a0c33`). `.github/workflows/scheduled-jobs.yml` wakes on the five distinct cron expressions and dispatches the jobs sharing each; `vercel.json` is deleted. Tests hold the workflow, the registry and the `on.schedule` triggers level, and keep crons out of `vercel.json` so the deploy blocker cannot be reintroduced. Staleness thresholds gained a 15-minute jitter allowance because Actions delivers scheduled runs best-effort — at the old threshold the 5-minute job went stale after 11 minutes and would have flapped. **Deploy confirmed unblocked:** `vercel deploy --prod` now builds and promotes where it was previously refused outright.
- **⚠️ THE ENV VARS ARE NOT OPTIONAL, demonstrated the hard way.** With the cron gate cleared, a production deploy on 2026-08-02 succeeded and took `amazing-skills.com` down — HTTP 500 / `MIDDLEWARE_INVOCATION_FAILED` on every request, boot logs reading verbatim *"Refusing to start: 8 environment problems in production."* Rolled back to the 2026-07-19 build within ~3 minutes; all three domains verified back at HTTP 200. **This project serves a live custom domain (`amazing-skills.com`, `www.amazing-skills.com`), so a production deploy is a real outage risk, not a staging exercise.** QLT-03's boot check worked exactly as designed — it refused to serve a half-configured platform rather than silently sending no email and 401ing every job. Set all 8 variables BEFORE the next deploy; there is no partial-credit path.
- **STILL OUTSTANDING (operator, not code):** (1) the 8 production variables — `RESEND_API_KEY`, `CRON_SECRET`, `TOKEN_ENCRYPTION_KEY`, `LEGAL_ENTITY_NAME`, `LEGAL_REGISTRATION_NUMBER`, `LEGAL_BUSINESS_ADDRESS`, `LEGAL_SUPPORT_EMAIL`, `LEGAL_INFORMATION_OFFICER_EMAIL` (the first two need ≥32 chars); (2) GitHub repository secret `CRON_SECRET` and variable `PRODUCTION_APP_URL`, without which the workflow fails its first step by design; (3) redeploy, then confirm liveness from `JobRun` check-ins and a 200 from `/api/v1/health/ready`. Only then is this tickable.
- **Files:** `src/lib/security/cron-auth.ts`, `src/lib/env.ts`, `.github/workflows/scheduled-jobs.yml`, `src/server/jobs/registry.ts`, `src/app/api/v1/health/ready/route.ts`
...
- **PARTIAL — machinery complete, one surface translated.** `next-intl` wired; locale resolves per request from **cookie → account → `Accept-Language` → English**; `User.locale` added (nullable, no backfill — NULL means *never chosen*, which is what keeps negotiation alive for existing accounts, where defaulting to `en` would have frozen them all); `lang` and `dir` now come from the resolved locale; language switcher in the footer with each option written in its own script. **Locales are an explicit bet, not a measurement** — the item said to sequence off signup data and there is none (all four accounts have `country` unset and the legacy Africa/Johannesburg timezone, `teacher_languages` empty), so Spanish is the first-market guess and Arabic exists to keep RTL honest. Revisit once real signups exist. **RTL needed more than `dir`:** 78 physical-direction Tailwind utilities converted to logical ones across 49 files. **Plurals are ICU, not `count === 1 ? "" : "s"`** — English has two plural categories, Arabic has six, and a test asserts the Arabic catalogue carries zero/one/two/few/many/other, without which 3–10 and 11–99 silently fall back to `other`. A catalogue-parity test fails the build on a missing key *and* on a catalogue copied from English. Two GLO-03 tests broke correctly — they pinned literal English `aria-label`s — and now assert structure instead. Verified in a browser: `Accept-Language` alone selects es/ar/en including quality ordering and regional fallback, and Arabic `/find-tutor` renders fully mirrored with the correct `one` plural form.
- **STILL OUTSTANDING, and it is most of the surface.** Only the marketplace, footer and skip link are extracted — the marketing homepage, dashboards, admin, booking, checkout and transactional email remain hardcoded English, so **"no user-facing string remains hardcoded" is not met**. Separately, **no URL-prefixed routing**: locales are not addressable, so each language cannot be indexed separately and GLO-02's deferred `hreflang` still has nothing to point at — which is where the commercial return on translation actually lives. That was a deliberate trade (moving all 71 pages under `app/[locale]/` in the same change as extraction and RTL is how a working app breaks) and is arranged so adding it later touches the i18n config and middleware, not the components.
- **Files:** `src/i18n/`, `messages/`, `src/app/layout.tsx`, `src/actions/locale.ts`, `src/features/i18n/components/language-switcher.tsx`, `next.config.ts`
...
- **PARTIAL — structure, gate and statement done; the rendered-page audit is not.** Skip link in the root layout with `#main-content` on all 38 `<main>` elements; navigation and contentinfo landmarks on the public marketplace pages; visually hidden `H2`s giving the filter and result regions a real heading outline. **Two defects that only measuring in a browser could find:** the skip link used `focus-visible:not-sr-only` and never appeared — it stayed clipped at `inset(50%)`, so it consumed a tab stop and delivered nothing (now `focus:`, since it must reveal on any focus, including focus restored after a route change); and plain anchors had **no focus indicator at all** — the base layer set an outline colour but left `outline-style: none`, so the wordmark, footer links and pagination showed nothing on focus, a WCAG 2.4.7 failure hidden by the fact that Buttons carry their own ring. Verified by pressing Tab rather than calling `.focus()`, because programmatic focus does not reliably match `:focus-visible` — which is precisely what disguised the first defect. **CI gate:** full `jsx-a11y` recommended set now runs in the existing lint step; of the fifteen issues it found, the false positives from library render-prop patterns were fixed by configuring `controlComponents` and three targeted disables rather than by weakening rules. Exactly one rule is off — `media-has-caption`, because every video is user-uploaded and no transcription pipeline exists — documented in one place and disclosed at `/accessibility`, with a test asserting nothing else is ever disabled. **Statement** at `/accessibility` claims WCAG 2.2 AA *partial* conformance, lists the real gaps (no captions, English-only UI, unassessed third-party classroom, out-of-scope payment pages) and states plainly that it is a self-assessment with no independent audit. **STILL OUTSTANDING:** an automated axe run against *rendered* pages, which needs a browser and a running server in CI — the static gate covers the regression risk that mattered here but cannot judge whether a label is meaningful. Keyboard completion of discover → book → pay is verified only as far as discovery; the booking and payment steps need a seeded student account and a sandbox payment to exercise honestly.
- **Files:** `src/app/layout.tsx`, `src/components/a11y/skip-link.tsx`, `src/app/accessibility/page.tsx`, `src/styles/globals.css`, `eslint.config.mjs`

...
const envSchema = z.object({
  ...
  // Paddle is the only rail (PAY-03). The client token is designed to sit in the browser, like
  // the Supabase publishable key, so it is NEXT_PUBLIC_ and must not be marked Sensitive in
  // Vercel: sensitive values are absent at build time and a NEXT_PUBLIC_ one would inline as
